...
`PERSONAL_REF` 填成了默认值，导致判断条件恒为真：**默认安装（自包含副本 + 个人层跟着副本走）
生成的 AGENTS.md 引用块自称「混合档」，还声称「第 3 条个人层在本机绝对路径」**——实际路径是
项目内的 `.vibe-rules/personal/`（相对路径）。ps1 侧顺序本来就是对的，属于 sh ↔ ps1 行为不对称。

### 修复

- `scripts/install.sh`：把 `PERSONAL_REF` 的默认填充挪到引用块判断**之后**（与 `install.ps1` 同序）。
  现在三种情况各自说对话：默认/`--no-personal` → 「自包含副本」；`--profile hybrid` → 「混合档」；
  `--link` → 「外链模式」。使用者重跑一次 `install.sh` / `update.sh` 即可刷新已装项目里的引用块

### 测试

- `tests/smoke.sh` / `tests/smoke.ps1` 各加 3 条回归断言：默认档引用块必须自称「自包含副本」
  且**不得出现**「混合档」；hybrid 档必须自称「混合档」。回归验证：把 `install.sh` 还原成修复前版本，
  冒烟立刻报 `❌ 引用块自称自包含副本（默认档不谎称混合档）` / `❌ 默认档引用块不提混合档`
- 断言数量变化：bash 280 → **283**，PS 181 → **184**（README 里的数字已同步，两个冒烟自带自检）
...
顺带把选项名里的数字（`--sha1` 这类）纳入提取范围，此前会被静默漏掉。

### 新增

- **lint ⑥ 反向**——脚本认、README 从没提的 `--flag` 同样报错（加了选项忘写文档）；
  维护者内部选项走白名单 `flag_doc_allowed()`，目前只有 `bump-version.sh` 的 `--no-changelog`
  （使用者抄不到这条命令，写进 README 只是噪音）
- **选项名允许数字**——`sh_flags` 的三处 grep、README 选项提取、反向匹配的字符类从 `[a-z]` 放宽到 `[a-z0-9]`：
  正则少写一个数字，`--sha1` 这类选项就会被**静默漏检**，比误报危险（冒烟负例 `--ghost2-flag` 顺带验证）

### 变更

- `tests/smoke.sh` 第 17 节新增反向漂移负例 + 内部选项白名单正例；bash 冒烟 273 项断言
- `README.md`：lint 表格 ⑥ 行改成"双向"（并在说明里点名 `--no-changelog` 白名单）
